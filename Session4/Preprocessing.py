...
# Generate the word library and train_raw, test_raw
def gen_data_and_vocab():
    # Get data from the original data
    ...
    # Store the library of vocab with freq > 10 (only from train data)
    ...
    # Store the data of train_data and test_data
...
def encode_data(data_path, vocab_path):
    with open(vocab_path) as f:
        vocab = dict([(word, word_ID + 2) for
                      word_ID, word in enumerate(f.read().splitlines())])
    with open(data_path) as f:
        documents = [(line.split('<fff>')[0], line.split('<fff>')[1], line.split('<fff>')[2])
                     for line in f.read().splitlines()]
    encoded_data = []
    unknown_ID = 0  # The word is not in the vocab
    padding_ID = 1  # The word "empty" is added
...
if __name__ == '__main__':
    gen_data_and_vocab()
    # Encode the train-raw and test-raw data
